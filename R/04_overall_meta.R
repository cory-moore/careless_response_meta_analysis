...
# 2. Subgroup analyses by sample characteristics and research context
#
...
# careless responding amount from each study. This approach answers the question
# "What is the typical total rate of careless responding identified in I-O 
# psychology research, regardless of detection approach?"
#
# Note: Method-related subgroup analyses (method_type, cr_method) are deliberately
# omitted for this approach since they are conceptually problematic when
# examining total CR rates across different method configurations.
#
...
tibble(
  ...
  write_csv("output/r_results/overall/overall_results.csv")

# Define and run subgroup analyses
cat("\nRunning subgroup analyses...\n")
codebook <- load_codebook()

# Deliberately excluding method-related variables from subgroup analyses
# for the Overall approach, focusing only on sample characteristics and context
subgroup_variables <- c("sample_source", "sample_recruitment", 
...
walk(subgroup_variables, ~{
  if (.x %in% colnames(overall_data)) {
    cat(glue("\nAnalyzing by {.x}:\n"))
    results <- run_subgroup_analysis(overall_data, .x, codebook)
