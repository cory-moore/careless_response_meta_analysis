...
def compute_proportions(data, cr_total):
    ...
    data['cr_proportion_total'] = cr_proportion_total
    return data

def compute_proportions_by_year(data, year):
    """Compute proportion of CR for a given year"""
    subset_df = data[data['year'] == year].copy()
    subset_df['cr_proportion_year'] = subset_df['cr_total_amount'] / subset_df['sample_size']
    subset_df['year'] = year
    subset_df = subset_df[['ID', 'year', 'sample_size', 'cr_total_amount', 'cr_proportion_year']]
    return subset_df
...
def compute_proportions_by_journal(data, journal_code):
    ...
    subset_df['cr_proportion_journal'] = subset_df['cr_total_amount'] / subset_df['sample_size']
    subset_df['journal_name'] = codebook['journal'][journal_code]
    subset_df = subset_df[['ID', 'journal_code', 'journal_name', 'sample_size', 'cr_total_amount', 'cr_proportion_journal']]
    return subset_df
...
def compute_proportions_by_sample_source(data, source_code):
    ...
    subset_df['cr_proportion_source'] = subset_df['cr_total_amount'] / subset_df['sample_size']
    subset_df['sample_source_name'] = codebook['sample_source'][source_code]
    subset_df = subset_df[['ID', 'sample_source', 'sample_source_name', 'sample_size', 'cr_total_amount', 'cr_proportion_source']]
    return subset_df
...
def compute_proportions_by_sample_method(data, method_code):
    ...
    subset_df['cr_proportion_method'] = subset_df['cr_total_amount'] / subset_df['sample_size']
    subset_df['sample_method_name'] = codebook['sample_method'][method_code]
    subset_df = subset_df[['ID', 'sample_method', 'sample_method_name', 'sample_size', 'cr_total_amount', 'cr_proportion_method']]
    return subset_df
...
def compute_proportions_by_sample_platform(data, platform_code):
    ...
    subset_df['cr_proportion_platform'] = subset_df['cr_total_amount'] / subset_df['sample_size']
    subset_df['sample_platform_name'] = codebook['sample_platform'][platform_code]
    subset_df = subset_df[['ID', 'sample_platform', 'sample_platform_name', 'sample_size', 'cr_total_amount', 'cr_proportion_platform']]
    return subset_df
...
def compute_cr_method_proportions(data, cr_method_code):
    ...
    subset_df['cr_proportion'] = subset_df['cr_amount'] / subset_df['sample_size']
    return subset_df
...
cr_proportions_total = cr_proportions_total[['ID', 'sample_size', 'cr_total_amount', 'cr_proportion_total']]
...
proportions_by_year = pd.concat(years_df)

journal_dfs = [compute_proportions_by_journal(data, code) for code in jounal_codes]
proportions_by_journal = pd.concat(journal_dfs)

source_dfs = [compute_proportions_by_sample_source(data, code) for code in source_codes]
proportions_by_source = pd.concat(source_dfs)

method_dfs = [compute_proportions_by_sample_method(data, code) for code in method_codes]
proportions_by_method = pd.concat(method_dfs)

platform_dfs = [compute_proportions_by_sample_platform(data, code) for code in platform_codes]
proportions_by_platform = pd.concat(platform_dfs)

cr_methods_dfs = [compute_cr_method_proportions(data, code) for code in cr_method_codes]
proportions_by_cr_method = pd.concat(cr_methods_dfs)

cr_type_dfs = [compute_cr_method_type_proportions(data, code) for code in cr_method_types]
proportions_by_cr_type = pd.concat(cr_type_dfs)


#----- Export ----------------------------------------------------------------------------------------------------------------------
writer = pd.ExcelWriter('results/careless_response_proportions.xlsx')

cr_proportions_total.to_excel(writer, 'cr_proportions_total', index=False)
proportions_by_year.to_excel(writer, 'cr_proportions_by_year', index=False)
proportions_by_journal.to_excel(writer, 'cr_proportions_by_journal', index=False)
proportions_by_source.to_excel(writer, 'cr_proportions_by_sample_source', index=False)
proportions_by_method.to_excel(writer, 'cr_proportions_by_sample_method', index=False)
proportions_by_platform.to_excel(writer, 'cr_proportions_by_sample_plat', index=False)
proportions_by_cr_method.to_excel(writer, 'cr_proportions_by_method', index=False)
proportions_by_cr_type.to_excel(writer, 'cr_proportions_by_method_type', index=False)
